...
const {rolesAllowedWithEmail, rolesAllowedWithMobile} = require("./constants")
...
exports.createUserValidator = [
  ...
  body('email')
    .if((value, { req }) => rolesAllowedWithEmail.includes(req.body.role))
  ...
  body('mobile_number')
    .if((value, { req }) => rolesAllowedWithMobile.includes(req.body.role))
...
exports.createOrderValidators = [
  ...
  body('inspection_manager')
    .notEmpty().withMessage("Inspection manager is required").bail()
    .custom(async (value, {req}) => {
      const userId = req.user._id;
      const inspectionManager = await User.findOne({_id: value, procurement_manager: userId}).select({role: 1, _id: 1});
  ...
  .custom(async (value, {req}) => {
    const userId = req.user._id;
      const checklist = await Checklist.findOne({_id: value, created_by: userId}).select({ _id: 1});
      if (!checklist) {
...
exports.answersValidations = [
  ...
  //   }),
  body("items")
    .notEmpty().withMessage("Please give answers for atleast required fields").bail()
    .isArray()
